Shader"MyShaders/Specular_Blinn_Phong"		//与普通的高光反射类似，只是使用了不同的光照模型
{
	Properties
	{
...
#pragma fragment frag
	...
	v2f vert(a2v _v)
	{
		...
		//tfrag.worldNormalDir = mul(_v.myNormal, (float3x3)unity_WorldToObject);		//将法线从模型空间转到世界空间
		tfrag.worldNormalDir = UnityObjectToWorldNormal(_v.myNormal);
		tfrag.worldVertex = mul(_v.myVertex,(float3x3)unity_WorldToObject);
		return tfrag;
	}
	fixed4 frag(v2f _f) :SV_Target
	{
		...
		fixed3 tLightDirNor = normalize(UnityWorldSpaceLightDir(_f.worldVertex));
		...
		fixed3 tCameraDirNor = normalize(UnityWorldSpaceViewDir(_f.worldVertex));
		fixed3 tHalfDirNor = normalize((tCameraDirNor + tLightDirNor) / 2);					//平分线：视野方向和平行光方向夹角
		fixed3 tSpecularColor = _LightColor0.rgb*_Specular.rgb*pow(max(0, dot(tNormalDirNor, tHalfDirNor)), _Gloss);//高光反射，使用
		//整体的平行光或者是自定义的颜色作为光源
